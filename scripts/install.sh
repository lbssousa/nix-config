...
#   0b. Clona e desbloqueia o repositório nix-keys via git-crypt
#   1.  Seleciona o host e o disco de destino
#   2.  Seleciona o perfil de particionamento (btrfs ou zfs)
...
#   8b. Registra o YubiKey para autenticação U2F (pamu2fcfg)
...
#                       (repositório privado com chaves age do sops-nix, criptografado
#                       com git-crypt). Se omitido, procura em ../nix-keys (irmão de
#                       nix-config) e, se não encontrado, pergunta interativamente.
#   --age-keys-backup   Caminho direto para o arquivo keys.txt da chave age de sistema
#                       (sops-nix). Alternativa ao --nix-keys-dir quando você tem apenas
#                       o arquivo da chave sem o repositório completo. Copiado para
#                       /persist/etc/sops/age/keys.txt no sistema instalado.
...
# Executa pamu2fcfg com os argumentos fornecidos.
# Se não estiver no PATH, baixa via nix-shell (pam_u2f já deve estar no cache
# do nixos-install).
_run_pamu2fcfg() {
...
Opções:
  --host              Nome do host NixOS (ex: barbudus, bigodon).
                      O atributo do flake usado no nixos-install é simplesmente
                      o nome do host (nixosConfigurations.<host>).
  ...
  --nix-keys-dir      Caminho para o clone local do repositório nix-keys
                      ...
                      git-crypt). Se omitido, procura em ../nix-keys (irmão de
                      nix-config). O repositório precisa já estar desbloqueado, ou
                      o script solicitará o desbloqueio via GPG ou chave simétrica.
  --age-keys-backup   Caminho direto para o arquivo keys.txt da chave age de sistema.
                      Alternativa a --nix-keys-dir quando você tem apenas o arquivo
                      sem o repositório completo. Copiado para
                      /persist/etc/sops/age/keys.txt no sistema instalado.
  --non-interactive   Não faz perguntas; falha se informações obrigatórias
                      não forem fornecidas via flags.
...
Exemplos:
  # Instalação totalmente interativa (recomendado):
  bash scripts/install.sh

  # Instalação não-interativa com perfil Btrfs, nix-keys em path customizado:
  bash scripts/install.sh \
    --host barbudus \
    --disk /dev/nvme0n1 \
...
NIX_KEYS_DIR=""                                          # definido/validado no passo 0b
NIX_KEYS_DIR_DEFAULT="$(dirname "$CONFIG_DIR")/nix-keys" # localização canônica (irmão de nix-config)
...
# ---------------------------------------------------------------------------
# 0b. Clonar e desbloquear o repositório nix-keys
# ---------------------------------------------------------------------------
# O repositório nix-keys é um repositório privado (SSH) separado do nix-config.
# Ele armazena chaves age do sops-nix, criptografadas com git-crypt:
#
#   nix-keys/
#     sops/age/keys.txt          ← chave age de SISTEMA (descriptografa secrets do NixOS,
#                                    ex: senha do Wi-Fi). Copiada para
#                                    /persist/etc/sops/age/keys.txt no passo 6b.
#     sops/age/<user>/keys.txt   ← chave age PESSOAL de cada usuário (descriptografa
#                                    secrets do Home Manager, ex: rclone). Copiada
#                                    pelo script de ativação do HM no primeiro login.
#
# O unlock via git-crypt requer o YubiKey (GPG) ou a chave simétrica exportada.
# Sem a chave de sistema, sops-nix não consegue descriptografar secrets durante a
# ativação do NixOS (Wi-Fi, etc.) e o serviço falha silenciosamente no boot.

echo
info "==> Passo 0b: Repositório nix-keys (chaves age do sops-nix)"

# Prioridade: flag --nix-keys-dir > default (irmão de nix-config) > interativo
if [[ -n "$OPT_NIX_KEYS_DIR" ]]; then
  if [[ ! -d "$OPT_NIX_KEYS_DIR" ]]; then
    die "--nix-keys-dir: diretório '$OPT_NIX_KEYS_DIR' não encontrado."
  fi
  NIX_KEYS_DIR="$OPT_NIX_KEYS_DIR"
  info "Usando nix-keys em: $NIX_KEYS_DIR (via --nix-keys-dir)"
elif [[ -d "$NIX_KEYS_DIR_DEFAULT/.git" ]]; then
  NIX_KEYS_DIR="$NIX_KEYS_DIR_DEFAULT"
  info "nix-keys encontrado em: $NIX_KEYS_DIR"
elif [[ "$OPT_NON_INTERACTIVE" == "true" ]]; then
  # Modo não-interativo sem nix-keys: apenas avisa. A chave pode vir via --age-keys-backup.
  if [[ -z "$OPT_AGE_KEYS_BACKUP" ]]; then
    warn "nix-keys não encontrado em $NIX_KEYS_DIR_DEFAULT e --age-keys-backup não especificado."
    warn "Secrets do sops-nix (Wi-Fi etc.) não estarão disponíveis sem a chave age de sistema."
    warn "Use --nix-keys-dir ou --age-keys-backup para fornecer a chave."
  fi
else
  echo
  warn "nix-keys não encontrado em $NIX_KEYS_DIR_DEFAULT."
  info "O repositório nix-keys contém as chaves age do sops-nix."
  info "Sem ele, os secrets de sistema (ex: Wi-Fi via sops-nix) não funcionarão."
  echo
  echo -ne "${BOLD}URL SSH do repositório nix-keys (Enter para pular): ${RESET}"
  read -r _nix_keys_url
  if [[ -n "$_nix_keys_url" ]]; then
    info "Clonando nix-keys em $NIX_KEYS_DIR_DEFAULT..."
    if git clone "$_nix_keys_url" "$NIX_KEYS_DIR_DEFAULT"; then
      NIX_KEYS_DIR="$NIX_KEYS_DIR_DEFAULT"
      success "nix-keys clonado em $NIX_KEYS_DIR."
    else
      warn "Falha ao clonar nix-keys. Verifique a URL e a conectividade SSH."
      warn "Você pode usar --age-keys-backup para fornecer a chave manualmente."
    fi
  else
    warn "nix-keys ignorado. Secrets do sops-nix não serão configurados nesta instalação."
  fi
fi

# Se o nix-keys foi localizado, atualizar o caminho da chave age de sistema
if [[ -n "$NIX_KEYS_DIR" ]]; then
  PRIVATE_AGE_KEY_REPO="$NIX_KEYS_DIR/sops/age/keys.txt"
fi

# Verificar se o nix-keys precisa de unlock via git-crypt
if [[ -n "$NIX_KEYS_DIR" && -d "$NIX_KEYS_DIR/.git-crypt" ]]; then
  # Verificar se já está desbloqueado: a chave age começa com 'AGE-SECRET-KEY-' quando
  # descriptografada. Se o arquivo for binário ou não tiver esse prefixo, está bloqueado.
  ...
  if [[ -f "$_age_system_key" ]]; then
    ...
    # Arquivo ainda não existe no índice — tratar como bloqueado para tentar unlock
    _nix_keys_locked=true
  fi

  if [[ "$_nix_keys_locked" == "true" ]]; then
    warn "nix-keys está bloqueado pelo git-crypt."
    warn "Necessário o YubiKey (GPG) ou a chave simétrica exportada para desbloquear."

    if ! command -v git-crypt >/dev/null 2>&1; then
      warn "git-crypt não encontrado. Execute antes de continuar:"
      warn "  nix-shell -p git-crypt gnupg"
      if [[ "$OPT_NON_INTERACTIVE" == "true" ]]; then
        die "Modo não-interativo: nix-keys bloqueado e git-crypt não disponível."
      fi
    else
      if [[ "$OPT_NON_INTERACTIVE" == "true" ]]; then
        die "Modo não-interativo: nix-keys bloqueado. Desbloqueie antes de rodar o script."
      fi

      echo
      info "Para desbloquear via YubiKey (GPG), certifique-se de que o YubiKey está inserido."
      info "Para desbloquear via chave simétrica, forneça o caminho do arquivo exportado."
      echo -ne "${BOLD}Caminho para a chave simétrica git-crypt (Enter para usar GPG via YubiKey): ${RESET}"
      read -r _git_crypt_key_path

      if [[ -n "$_git_crypt_key_path" ]]; then
        (cd "$NIX_KEYS_DIR" && git-crypt unlock "$_git_crypt_key_path") \
          && success "nix-keys desbloqueado com sucesso." \
          || die "Falha ao desbloquear nix-keys. Verifique a chave simétrica."
      else
        info "Tentando desbloquear via GPG (YubiKey)..."
        (cd "$NIX_KEYS_DIR" && git-crypt unlock) \
          && success "nix-keys desbloqueado via GPG com sucesso." \
          || die "Falha ao desbloquear nix-keys via GPG. Verifique que o YubiKey está inserido e o GPG configurado."
      fi
    fi
  else
    success "nix-keys já está desbloqueado."
  fi
fi
...
HOST="$OPT_HOST"
# O atributo do flake é simplesmente o nome do host (nixosConfigurations.<host>).
# Não há variantes por desktop: o ambiente gráfico é configurado diretamente nos
# módulos compartilhados (dendritic/features/nixos-modules.nix).
...
# ---------------------------------------------------------------------------
# 6b. Copiar chave age de sistema do sops-nix para /persist
# ---------------------------------------------------------------------------
# A chave age de sistema é necessária para que o sops-nix possa descriptografar
# secrets durante a ativação do NixOS (ex: perfis Wi-Fi, certificados).
# Ela deve existir em /persist/etc/sops/age/keys.txt antes do primeiro boot.
#
# Fonte em ordem de prioridade:
#   1. --age-keys-backup <arquivo>   (especificado via flag)
#   2. nix-keys/sops/age/keys.txt   (repositório desbloqueado no passo 0b)
#   3. Pergunta interativa           (caminho manual)
#
# Nota: a chave age PESSOAL de cada usuário (nix-keys/sops/age/<user>/keys.txt)
# é copiada pelo script de ativação do Home Manager no primeiro login — não é
# necessário tratá-la aqui.

echo
info "==> Passo 6b: Copiar chave age de sistema do sops-nix para /persist"
...
if [[ -z "$OPT_AGE_KEYS_BACKUP" && -f "$_DEFAULT_AGE_KEYS_BACKUP" ]]; then
  ...
  info "Usando chave age de $NIX_KEYS_DIR (desbloqueado no passo 0b)."
fi

if [[ "$OPT_NON_INTERACTIVE" != "true" && -z "$OPT_AGE_KEYS_BACKUP" ]]; then
  info "A chave age de sistema é necessária para que sops-nix descriptografe secrets"
  info "(ex: senha do Wi-Fi) na ativação do NixOS. Sem ela, o sistema inicializa mas"
  info "os secrets não serão aplicados até que a chave seja copiada manualmente."
  echo -ne "${BOLD}Caminho para a chave age de sistema keys.txt (Enter para pular): ${RESET}"
...
# ---------------------------------------------------------------------------
# 8b. Registrar YubiKey para autenticação U2F (pamu2fcfg)
# ---------------------------------------------------------------------------
# O sistema usa pam_u2f como único método de autenticação para sudo/run0/pkexec
# (sem fallback de senha). Sem o arquivo /persist/etc/u2f-mappings, será impossível
# escalonar privilégios após o boot.

echo
info "==> Passo 8b: Registrar YubiKey para autenticação U2F (pamu2fcfg)"

_U2F_AUTHFILE="/mnt/persist/etc/u2f-mappings"

# Verificar se o sistema instalado configura pam_u2f para sudo
...
if [[ "$_U2F_CONFIGURED" != "true" ]]; then
  info "Sistema instalado não usa pam_u2f para sudo. Pulando."
else
  # Encontrar usuários do grupo wheel no sistema instalado
  _WHEEL_USERS_U2F=()
  ...
  if [[ ${#_WHEEL_USERS_U2F[@]} -eq 0 ]]; then
    warn "Nenhum usuário no grupo wheel encontrado no sistema instalado. Pulando."
  else
    info "Usuários wheel: ${_WHEEL_USERS_U2F[*]}"

    # Verificar se a YubiKey está conectada (vendor ID 1050 = Yubico)
    if ! lsusb 2>/dev/null | grep -qi "yubico"; then
      warn "YubiKey não detectada (lsusb não encontrou dispositivo Yubico)."
      warn "Sem o arquivo $( basename "$_U2F_AUTHFILE" ), sudo/run0/pkexec serão negados após o boot."
      warn "Consulte INSTALLATION.md → 'Lockout de sudo/run0' para recuperação."
    else
      info "YubiKey detectada. Iniciando registro para cada usuário wheel..."
      mkdir -p "$(dirname "$_U2F_AUTHFILE")"
      _u2f_first=true

      for _wu in "${_WHEEL_USERS_U2F[@]}"; do
        _wu="${_wu// /}"  # remove espaços residuais do split
        [[ -z "$_wu" ]] && continue

        echo
        info "Registrando usuário '$_wu' — quando o LED da YubiKey piscar, toque-a."

        if [[ "$_u2f_first" == "true" ]]; then
          if _run_pamu2fcfg -u "$_wu" > "$_U2F_AUTHFILE"; then
            success "YubiKey registrada para '$_wu'."
            _u2f_first=false
          else
            warn "Falha ao registrar YubiKey para '$_wu'. Tente manualmente:"
      ...
      if [[ -f "$_U2F_AUTHFILE" && -s "$_U2F_AUTHFILE" ]]; then
        success "Arquivo $( basename "$_U2F_AUTHFILE" ) criado:"
        while IFS= read -r _u2f_line; do
          info "  ${_u2f_line%%:*}: [credencial registrada]"
        done < "$_U2F_AUTHFILE"
      else
        warn "ATENÇÃO: $_U2F_AUTHFILE não foi criado ou está vazio."
        warn "sudo, run0 e pkexec serão negados após o boot."
        warn "Consulte INSTALLATION.md → 'Lockout de sudo/run0' para recuperação."
      fi
    fi
  fi
...
echo -e "        ${CYAN}# Exemplo para o usuário abutre:${RESET}"
...
echo -e "        ${CYAN}just home switch   # a ativação copia a chave pessoal automaticamente${RESET}"
echo
if [[ "$_U2F_CONFIGURED" == "true" ]] && [[ ! -s "$_U2F_AUTHFILE" ]]; then
  echo -e "  ${YELLOW}${BOLD}⚠  ATENÇÃO: YubiKey não registrada (passo 8b incompleto).${RESET}"
  echo -e "  ${YELLOW}   sudo, run0 e pkexec usarão senha até o registro ser feito.${RESET}"
  echo -e "  ${YELLOW}   Registre a YubiKey ANTES de desmontar (com a YubiKey inserida):${RESET}"
  echo
  if [[ ${#_WHEEL_USERS_U2F[@]} -gt 0 ]]; then
    ...
    echo -e "    ${CYAN}pamu2fcfg -u <usuario> > /mnt/persist/etc/u2f-mappings${RESET}"
    echo -e "    ${CYAN}pamu2fcfg -u <outro-usuario> >> /mnt/persist/etc/u2f-mappings${RESET}"
  fi
  echo
  echo -e "  ${YELLOW}   Consulte INSTALLATION.md → 'Passo 10' para instruções completas.${RESET}"
  echo
fi
