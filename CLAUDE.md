...
Since `/home` has its own persistent Btrfs subvolume (see disko.nix), packages and dotfiles installed via the standalone path survive a reboot on their own — no special preservation handling is needed for `.local/state/nix/profiles`, `.local/state/home-manager`, or `~/.nix-profile`. The NixOS-coupled path still gets HM re-activated automatically on every boot (a side effect of NixOS re-running its activation scripts, not just on `nixos-rebuild switch`); the standalone path only updates when you explicitly run `just home switch`. Use it for fast iteration on dotfiles/packages; run a real `just switch` (or wait for the next one) to also apply system-level (NixOS) changes.
