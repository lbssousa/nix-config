...
- Aplicações GUI instaladas preferencialmente via Nix; Flatpak apenas para apps sem equivalente no nixpkgs
- Sistema efêmero (preservation) com raiz limpa a cada boot
...
- Sistema efêmero via nix-community/preservation
...
| `@persist` | `/persist` | Preservado — dados persistentes do sistema (preservation) |
...
**Preservation — estratégia:**

- A raiz tmpfs é sempre "limpa" ao boot — não requer snapshot ou rollback
- Arquivos importantes são preservados em `/persist` via bind mounts (preservation)
...
- No perfil GNOME, apps padrão são instalados via Nix (Firefox, Brave, Ghostty, Bitwarden, etc.); Flatpak apenas para 5 apps sem equivalente no nixpkgs
...
- Gerenciado declarativamente via `nix-flatpak` (input `gmodena/nix-flatpak`)

**Apps instalados declarativamente (sem equivalente no nixpkgs):**
...
**Flatpaks recomendados para instalar manualmente:**
...
6. Implementado via `nix-community/preservation` (módulo `preservation.preserveAt."/persist"`)
